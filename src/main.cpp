...
struct RunPinnedTaskLoopTask : enki::IPinnedTask {
  void Execute() override {
    while (task_scheduler->GetIsRunning() && execute) {
      task_scheduler
          ->WaitForNewPinnedTasks();  // this thread will 'sleep' until there
                                      // are new pinned tasks
...
glm::vec2 encodeNormalOctahedron(glm::vec3 n) {
  ...
  return (n.z < 0.0f)
             ? ((glm::vec2(1.0) - abs(glm::vec2(p.y, p.x))) * sign_not_zero(p))
             : p;
}
...
bool project_sphere(glm::vec3 C, float r, float znear, float P00, float P11,
                    glm::vec4& aabb) {
  ...
  aabb = glm::vec4(aabb.x, aabb.w, aabb.z, aabb.y) *
...
int main(int argc, char** argv) {
  ...
  // Callback register
  window.register_os_messages_callback(input_os_messages_callback,
                                       &input_handler);
  ...
  {
    {
      ...
      cstring frame_graph_path = temporary_name_buffer.append_use_f(
          HELIX_FRAMEGRAPH_FOLDER "cull_graph.json");
      ...
      cstring full_screen_pipeline_path = temporary_name_buffer.append_use_f(
          "%s/%s", HELIX_SHADER_FOLDER, "programs/fullscreen.json");
      resources_loader.load_program(full_screen_pipeline_path);

      temporary_name_buffer.clear();
      cstring culling_pipeline_path = temporary_name_buffer.append_use_f(
          "%s/%s", HELIX_SHADER_FOLDER, "programs/culling.json");
...
#else
      cstring meshlet_pipeline_path = temporary_name_buffer.append_use_f(
          "%s/%s", HELIX_SHADER_FOLDER, "programs/meshlet_ext.json");
#endif  // NVIDIA
      resources_loader.load_program(meshlet_pipeline_path);

      temporary_name_buffer.clear();
      cstring pbr_pipeline_path = temporary_name_buffer.append_use_f(
          "%s/%s", HELIX_SHADER_FOLDER, "programs/pbr_lighting.json");
      resources_loader.load_program(pbr_pipeline_path);

      temporary_name_buffer.clear();
      cstring light_debug_pipeline_path = temporary_name_buffer.append_use_f(
          "%s/%s", HELIX_SHADER_FOLDER, "programs/debug.json");
  ...
  scene->init(&renderer, allocator, &frame_graph, &stack_allocator,
              &async_loader);
  // scene->load(gltf_file, gltf_base_path, allocator, &stack_allocator,
  // &async_loader);
  ...
  camera.init({0.0f, 0.0f, 0.0f},
              gpu.swapchain_width * 1.0f / gpu.swapchain_height);
  ...
  while (!window.requested_exit) {
    ...
    if (!window.minimized) {
      ...
      if (async_loader.file_load_requests.size == 0 && checksz) {
        checksz = false;
        HINFO("Finished uploading textures in {} seconds",
              Time::from_seconds(absolute_begin_frame_tick));
...

      {
        ...
        /*  if (ImGui::Begin("Scene")) {
              ...
              ImGui::SliderFloat3("Light position", light_position.raw,
          -30.f, 30.f); ImGui::InputFloat3("Camera position",
          camera.position.raw); ImGui::InputFloat("Light range", &light_range);
        ...
        if (ImGui::Begin("Viewport")) {
          ...
          camera.aspect_ratio =
              viewportPanelSize.x * 1.0f / viewportPanelSize.y;
          camera.hovering_viewport = ImGui::IsWindowHovered();
        }
        ImGui::End();

        if (ImGui::Begin("Scene Settings")) {
          ImGui::Text("x: %f, y: %f, z: %f", camera.position.x,
                      camera.position.y, camera.position.z);
          ImGui::Text("AABB \t minx: %f, miny: %f, maxx: %f, maxy: %f",
                      scene->tester.x, scene->tester.y, scene->tester.z,
                      scene->tester.w);
          ImGui::SliderFloat3("Light position", (float*)&light_position, -30.f,
                              30.f);
      ...
      {
        {
          ...
          // (LightNode*)scene->node_pool.access_node({0, NodeType::LightNode});

          // cb_data->light_position =
          // glm::vec4(light_node->world_transform.translation.x,
          // light_node->world_transform.translation.y,
          // light_node->world_transform.translation.z, 1.0f);
          ...
          scene_data.aspect_ratio =
              gpu.swapchain_width * 1.f / gpu.swapchain_height;
          ...
          scene_data.frustum_planes[0] =
              normalize_plane(projection_transpose[3] +
                              projection_transpose[0]);  // x + w  < 0;
          scene_data.frustum_planes[1] =
              normalize_plane(projection_transpose[3] -
                              projection_transpose[0]);  // x - w  < 0;
          scene_data.frustum_planes[2] =
              normalize_plane(projection_transpose[3] +
                              projection_transpose[1]);  // y + w  < 0;
          scene_data.frustum_planes[3] =
              normalize_plane(projection_transpose[3] -
                              projection_transpose[1]);  // y - w  < 0;
          scene_data.frustum_planes[4] =
              normalize_plane(projection_transpose[3] +
                              projection_transpose[2]);  // z + w  < 0;
          scene_data.frustum_planes[5] =
              normalize_plane(projection_transpose[3] -
                              projection_transpose[2]);  // z - w  < 0;
          ...
          // light_cb_data->camera_position_texture_index =
          // glm::vec4(camera.position.x, camera.position.y, camera.position.z,
          // scene->light_texture.handle.index);

          glm::mat4 model = glm::mat4(1.0f);
          // model = glm::translate(model,
          // light_node->world_transform.translation); light_cb_data->model =
          // model;
        }
