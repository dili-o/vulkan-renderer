...
static FrameGraphResourceType string_to_resource_type(cstring input_type) {
  ...
  if (strcmp(input_type, "reference") == 0) {
    // This is used for resources that need to create an edge but are not
    // actually used by the render pass
...
static void compute_edges(FrameGraph* frame_graph, FrameGraphNode* node,
                          u32 node_index) {
  for (u32 r = 0; r < node->inputs.size; ++r) {
    FrameGraphResource* input_resource =
        frame_graph->access_resource(node->inputs[r]);

    FrameGraphResource* output_resource =
        frame_graph->get_resource(input_resource->name);
    if (output_resource == nullptr && !input_resource->resource_info.external) {
      // TODO(marco): external resources
      HASSERT_MSG(false,
                  "Requested resource is not produced by any node and is not "
                  "external.");
      continue;
    }
    ...
    FrameGraphNode* parent_node =
        frame_graph->access_node(input_resource->producer);
...
static void create_framebuffer(FrameGraph* frame_graph, FrameGraphNode* node) {
  ...
  for (u32 r = 0; r < node->outputs.size; ++r) {
    FrameGraphResource* resource =
        frame_graph->access_resource(node->outputs[r]);
    ...
    if (width == 0) {
      ...
      scale_width =
          info.texture.scale_width > 0.f ? info.texture.scale_width : 1.f;
    } else {
    ...
    if (height == 0) {
      ...
      scale_height =
          info.texture.scale_height > 0.f ? info.texture.scale_height : 1.f;
    } else {
  ...
  node->framebuffer =
      frame_graph->builder->device->create_framebuffer(framebuffer_creation);
}
...
static void create_render_pass(FrameGraph* frame_graph, FrameGraphNode* node) {
  ...
  // NOTE(marco): first create the outputs, then we can patch the input
  // resources with the right handles
  for (u32 i = 0; i < node->outputs.size; ++i) {
    FrameGraphResource* output_resource =
        frame_graph->access_resource(node->outputs[i]);

    FrameGraphResourceInfo& info = output_resource->resource_info;

    if (output_resource->type == FrameGraphResourceType_Attachment) {
      if (info.texture.format == VK_FORMAT_D32_SFLOAT) {
        render_pass_creation.set_depth_stencil(
            info.texture.format,
        ...
        render_pass_creation.add_attachment(
            info.texture.format, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL,
            info.texture.load_op);
      }
    }
  }
  ...
  for (u32 i = 0; i < node->inputs.size; ++i) {
    FrameGraphResource* input_resource =
        frame_graph->access_resource(node->inputs[i]);
  ...
  node->render_pass =
      frame_graph->builder->device->create_render_pass(render_pass_creation);
}
...
void FrameGraphResourceCache::shutdown() {
  ...
  while (it.is_valid()) {
    ...
    if (resource->type == FrameGraphResourceType_Texture ||
      ...
      Texture* texture =
          device->access_texture(resource->resource_info.texture.handle);
      device->destroy_texture(texture->handle);
    } else if (resource->type == FrameGraphResourceType_Buffer) {
      // TODO: Right now buffer resources in framegraphs are handled separately
      // by the Scene class
      // Buffer* buffer =
      // device->access_buffer(resource->resource_info.buffer.handle);
...
void FrameGraphBuilder::register_render_pass(
    cstring name, FrameGraphRenderPass* render_pass) {
  ...
  FrameGraphNode* node = (FrameGraphNode*)node_cache.nodes.access_resource(
      node_cache.node_map.get(it));
  node->graph_render_pass = render_pass;
}

FrameGraphResourceHandle FrameGraphBuilder::create_node_output(
    const FrameGraphResourceCreation& creation, FrameGraphNodeHandle producer) {
  ...
  FrameGraphResource* resource =
      resource_cache.resources.get(resource_handle.index);
  ...
  if (creation.type != FrameGraphResourceType_Reference) {
    ...
    resource_cache.resource_map.insert(
        hash_bytes((void*)resource->name, strlen(creation.name)),
...
FrameGraphResourceHandle FrameGraphBuilder::create_node_input(
    const FrameGraphResourceCreation& creation) {
  FrameGraphResourceHandle resource_handle = {k_invalid_index};
...
FrameGraphNodeHandle FrameGraphBuilder::create_node(
  ...
  FrameGraphNode* node =
      (FrameGraphNode*)node_cache.nodes.access_resource(node_handle.index);
  ...
  node_cache.node_map.insert(hash_bytes((void*)node->name, strlen(node->name)),
                             node_handle.index);

  // NOTE(marco): first create the outputs, then we can patch the input
  // resources with the right handles
  for (u32 i = 0; i < creation.output_creations.size; ++i) {
    const FrameGraphResourceCreation& output_creation =
        creation.output_creations[i];

    FrameGraphResourceHandle output =
        create_node_output(output_creation, node_handle);

    node->outputs.push(output);
  }

  for (u32 i = 0; i < creation.input_creations.size; ++i) {
    const FrameGraphResourceCreation& input_creation =
        creation.input_creations[i];
...
FrameGraphNode* FrameGraphBuilder::access_node(FrameGraphNodeHandle handle) {
  FrameGraphNode* node =
      (FrameGraphNode*)node_cache.nodes.access_resource(handle.index);

  return node;
}

FrameGraphResource* FrameGraphBuilder::get_resource(cstring name) {
  FlatHashMapIterator it =
      resource_cache.resource_map.find(hash_calculate(name));
  if (it.is_invalid()) {
    return nullptr;
  }

  FrameGraphResource* resource =
      resource_cache.resources.get(resource_cache.resource_map.get(it));

  return resource;
}

FrameGraphResource* FrameGraphBuilder::access_resource(
...
void FrameGraph::parse(cstring file_path, StackAllocator* temp_allocator) {
  ...
  for (sizet i = 0; i < passes.size(); ++i) {
    ...
    node_creation.output_creations.init(temp_allocator,
                                        (u32)pass_outputs.size());
    ...
    for (sizet ii = 0; ii < pass_inputs.size(); ++ii) {
      ...
      input_creation.name =
          string_buffer.append_use_f("%s", input_name.c_str());
    ...
    for (sizet oi = 0; oi < pass_outputs.size(); ++oi) {
      ...
      output_creation.name =
          string_buffer.append_use_f("%s", output_name.c_str());
      ...
      switch (output_creation.type) {
        ...
        case FrameGraphResourceType_Attachment: {
          ...
          output_creation.resource_info.texture.format =
              util_string_to_vk_format(format.c_str());
          ...
          if (TextureFormat::has_depth(
            ...
            if (clear_color_array.is_array()) {
              for (u32 c = 0; c < clear_color_array.size(); ++c) {
                output_creation.resource_info.texture.clear_values[c] =
                    clear_color_array[c];
              }
            } else {
              if (output_creation.resource_info.texture.load_op ==
                  RenderPassOperation::Clear) {
                HERROR(
                    "Error parsing output texture {}: load operation is clear, "
                    "but clear color not specified. Defaulting to 0,0,0,0.",
                    output_creation.name);
              }
...
void FrameGraph::compile() {
  ...
  for (u32 i = 0; i < nodes.size; ++i) {
    ...
    // NOTE(marco): we want to clear all edges first, then populate them. If we
    // clear them inside the loop below we risk clearing the list after it has
    // already been used by one of the child nodes
    node->edges.clear();
  }
  ...
  // NOTE(marco): allocations and deallocations are used for verification
  // purposes only
  ...
  deallocations.init(&linear_allocator, (u32)resource_count,
                     (u32)resource_count);
  for (u32 i = 0; i < resource_count; ++i) {
  ...
  for (u32 i = 0; i < nodes.size; ++i) {
    ...
    for (u32 j = 0; j < node->inputs.size; ++j) {
      FrameGraphResource* input_resource =
          builder->access_resource(node->inputs[j]);
      FrameGraphResource* resource =
          builder->access_resource(input_resource->output_handle);
  ...
  for (u32 i = 0; i < nodes.size; ++i) {
      ...
      if (!resource->resource_info.external &&
        ...
        if (resource->type == FrameGraphResourceType_Attachment) {
          ...
          if (info.texture.width == 0 || info.texture.height == 0) {
            info.texture.width = (u32)(builder->device->swapchain_width *
                                       info.texture.scale_width);
            info.texture.height = (u32)(builder->device->swapchain_height *
                                        info.texture.scale_height);
          }
          ...
          if (free_list.size > 0) {
            ...
            texture_creation.set_data(nullptr)
                ...
                .set_format_type(info.texture.format,
                                 TextureType::Enum::Texture2D)
                .set_size(info.texture.width, info.texture.height,
                          info.texture.depth)
                .set_flags(1, TextureFlags::RenderTarget_mask);
            TextureHandle handle =
                builder->device->create_texture(texture_creation);
        ...
        HDEBUG("Output {} allocated on node {}", resource->name,
               nodes[i].index);
      }
    }
    ...
    for (u32 j = 0; j < node->inputs.size; ++j) {
      ...
      if (!resource->resource_info.external && resource->ref_count == 0) {
        ...
        HDEBUG("Output {} deallocated on node {}", resource->name,
               nodes[i].index);
      }
    }
  }
...
void FrameGraph::render(u32 current_frame_index, CommandBuffer* gpu_commands,
  ...
  for (u32 n = 0; n < nodes.size; ++n) {
    ...
    if (!node->graph_render_pass) {
      HCRITICAL("Node: \"{}\" does not have a registered graph_render_pass!",
                node->name);
    }
    ...
    if (node->compute) {
      ...
      for (u32 i = 0; i < node->inputs.size; ++i) {
        FrameGraphResource* input_resource =
            builder->access_resource(node->inputs[i]);
        FrameGraphResource* resource =
            builder->access_resource(input_resource->output_handle);
        ...
        if (input_resource->type == FrameGraphResourceType_Texture) {
          Texture* texture = gpu_commands->device->access_texture(
              resource->resource_info.texture.handle);

          util_add_image_barrier(gpu_commands->device, gpu_commands->vk_handle,
                                 texture, RESOURCE_STATE_SHADER_RESOURCE, 0, 1,
      ...
      for (u32 o = 0; o < node->outputs.size; ++o) {
        FrameGraphResource* resource =
            builder->access_resource(node->outputs[o]);
        ...
        if (resource->type == FrameGraphResourceType_Attachment) {
          ...
          if (TextureFormat::has_depth(texture->vk_format)) {
            ...
            util_add_image_barrier(
                gpu_commands->device, gpu_commands->vk_handle, texture,
      ...
      node->graph_render_pass->post_render(current_frame_index, gpu_commands,
                                           this);
      ...
      for (u32 i = 0; i < node->inputs.size; ++i) {
        ...
        if (input_resource->type == FrameGraphResourceType_Texture) {
          ...
          if (!TextureFormat::has_depth_or_stencil(texture->vk_format)) {
            util_add_image_barrier(gpu_commands->device,
                                   gpu_commands->vk_handle, texture,
      ...
      for (u32 o = 0; o < node->outputs.size; ++o) {
        ...
        if (resource->type == FrameGraphResourceType_Attachment) {
          ...
          if (TextureFormat::has_depth(texture->vk_format)) {
            ...
            gpu_commands->clear_depth_stencil(clear_color[0],
                                              (u8)clear_color[1]);
          } else {
            util_add_image_barrier(gpu_commands->device,
                                   gpu_commands->vk_handle, texture,
                                   RESOURCE_STATE_RENDER_TARGET, 0, 1, false);
            f32* clear_color = resource->resource_info.texture.clear_values;
            gpu_commands->clear(clear_color[0], clear_color[1], clear_color[2],
                                clear_color[3], o);
          }
        }
      }
...
FrameGraphResource* FrameGraph::access_resource(
    FrameGraphResourceHandle handle) {
