...
static TextureHandle create_texture_from_file(GpuDevice& gpu, cstring filename,
                                              cstring name,
                                              bool create_mipmaps) {
  if (filename) {
...
ProgramCreation& ProgramCreation::add_pipeline(
    const PipelineCreation& pipeline) {
...
void Renderer::begin_frame() { gpu->new_frame(); }
  ...
  if (ImGui::Begin("Renderer Resources")) {
    if (ImGui::BeginTabBar("MyTabBar", ImGuiTabBarFlags_None)) {
      if (ImGui::BeginTabItem("Textures")) {
        Helix::FlatHashMapIterator it =
            resource_cache.textures.iterator_begin();
        while (it.is_valid()) {
          ...
          if (ImGui::TreeNode(texture->name)) {
            ...
            ImGui::Image((ImTextureID)&texture->handle,
                         {viewportPanelSize.x, viewportPanelSize.x});
            ImGui::TreePop();
          }
      ...
      if (ImGui::BeginTabItem("Buffers")) {
        ...
        while (it.is_valid()) {
          ...
          if (ImGui::TreeNode(buffer->desc.name)) {
            ...
            // ImGui::Text("Type: %s",
            // string_VkBufferUsageFlags(buffer->desc.type_flags));
            ImGui::TreePop();
          }
      ...
      if (ImGui::BeginTabItem("Samplers")) {
        Helix::FlatHashMapIterator it =
            resource_cache.samplers.iterator_begin();
        while (it.is_valid()) {
          Helix::SamplerResource* sampler = resource_cache.samplers.get(it);
          if (ImGui::TreeNode(sampler->desc.name)) {
            ImGui::Text("Min Filter: %s",
                        string_VkFilter(sampler->desc.min_filter));
            ImGui::Text("Mag Filter: %s",
                        string_VkFilter(sampler->desc.mag_filter));
            ImGui::Text("Mip Filter: %s",
                        string_VkSamplerMipmapMode(sampler->desc.mip_filter));

            ImGui::Text(
                "Address Mode U: %s",
                string_VkSamplerAddressMode(sampler->desc.address_mode_u));
            ImGui::Text(
                "Address Mode V: %s",
                string_VkSamplerAddressMode(sampler->desc.address_mode_v));
            ImGui::Text(
                "Address Mode W: %s",
                string_VkSamplerAddressMode(sampler->desc.address_mode_w));
            ImGui::TreePop();
          }
...
BufferResource* Renderer::create_buffer(VkBufferUsageFlags type,
...
TextureResource* Renderer::create_texture(cstring name, cstring filename,
  ...
  if (texture) {
    TextureHandle handle =
        create_texture_from_file(*gpu, filename, name, create_mipmaps);
    texture->handle = handle;
    gpu->query_texture(handle, texture->desc);
    texture->references = 1;
    texture->name = name;
...
Program* Renderer::create_program(const ProgramCreation& creation) {
  ...
  if (program) {
    program->passes.init(gpu->allocator, creation.num_creations,
                         creation.num_creations);
    ...
    for (uint32_t i = 0; i < creation.num_creations; ++i) {
      ...
      if (pass_creation.name != nullptr) {
        char* cache_path = pipeline_cache_path.append_use_f(
            "%s%s.cache", HELIX_SHADER_FOLDER "cache/", pass_creation.name);
      ...
      program->name_hash_to_index.insert(hash_calculate(pass_creation.name),
                                         (u32)i);
...
/*DescriptorSetHandle Renderer::create_descriptor_set(CommandBuffer*
command_buffer, Material* material, DescriptorSetCreation& ds_creation) {
    HASSERT(material != nullptr);

    DescriptorSetLayoutHandle set_layout =
material->program->passes[0].descriptor_set_layout;
...
// TODO:
static void generate_mipmaps(Helix::Texture* texture, Helix::CommandBuffer* cb,
                             bool from_transfer_queue) {
  using namespace Helix;

  if (texture->mip_level_count > 1) {
    util_add_image_barrier(cb->device, cb->vk_handle, texture->vk_image,
                           from_transfer_queue ? RESOURCE_STATE_COPY_SOURCE
  ...
  for (int mip_index = 1; mip_index < texture->mip_level_count; ++mip_index) {
    util_add_image_barrier(cb->device, cb->vk_handle, texture->vk_image,
                           RESOURCE_STATE_UNDEFINED, RESOURCE_STATE_COPY_DEST,
                           mip_index, 1, false);
    ...
    vkCmdBlitImage(cb->vk_handle, texture->vk_image,
    ...
    util_add_image_barrier(cb->device, cb->vk_handle, texture->vk_image,
                           RESOURCE_STATE_COPY_DEST, RESOURCE_STATE_COPY_SOURCE,
                           mip_index, 1, false);
  }
  ...
  if (from_transfer_queue) {
    util_add_image_barrier(
        cb->device, cb->vk_handle, texture->vk_image,
        (texture->mip_level_count > 1) ? RESOURCE_STATE_COPY_SOURCE
                                       : RESOURCE_STATE_COPY_DEST,
        RESOURCE_STATE_SHADER_RESOURCE, 0, texture->mip_level_count, false);
  } else {
    util_add_image_barrier(
        cb->device, cb->vk_handle, texture->vk_image, RESOURCE_STATE_UNDEFINED,
        RESOURCE_STATE_SHADER_RESOURCE, 0, texture->mip_level_count, false);
  }
}
...
void Renderer::add_texture_update_commands(u32 thread_id) {
  ...
  for (u32 i = 0; i < num_textures_to_update; ++i) {
    ...
    util_add_image_barrier(cb->device, cb->vk_handle, texture->vk_image,
                           RESOURCE_STATE_COPY_DEST, RESOURCE_STATE_COPY_SOURCE,
                           0, 1, false, gpu->vulkan_transfer_queue_family,
