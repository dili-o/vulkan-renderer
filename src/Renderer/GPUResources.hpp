...
//        return index == handle.index; // Can't think of a reason why we want
//        to check if generations match.
//    }
//};
...
static const u8 k_max_descriptor_set_layouts =
    8;  // Maximum number of layouts in the pipeline.
static const u8 k_max_shader_stages =
    5;  // Maximum simultaneous shader stages. Applicable to all different type
        // of pipelines.
static const u8 k_max_descriptors_per_set =
    16;  // Maximum list elements for both descriptor set layout and descriptor
         // sets.
...
struct DepthStencilCreation {
  ...
  DepthStencilCreation()
      : depth_enable(0), depth_write_enable(0), stencil_enable(0) {}
...
struct BlendState {
  ...
  BlendState& set_color(VkBlendFactor source_color,
                        VkBlendFactor destination_color,
                        VkBlendOp color_operation);
  BlendState& set_alpha(VkBlendFactor source_color,
                        VkBlendFactor destination_color,
                        VkBlendOp color_operation);
...
struct SamplerCreation {
  ...
  SamplerCreation& set_min_mag_mip(VkFilter min, VkFilter mag,
                                   VkSamplerMipmapMode mip);
  SamplerCreation& set_address_mode_u(VkSamplerAddressMode u);
  SamplerCreation& set_address_mode_uv(VkSamplerAddressMode u,
                                       VkSamplerAddressMode v);
  SamplerCreation& set_address_mode_uvw(VkSamplerAddressMode u,
                                        VkSamplerAddressMode v,
...
struct ShaderStateCreation {
  ...
  ShaderStateCreation& add_stage(cstring code, u32 code_size,
...
struct DescriptorSetLayoutCreation {
  //
  // A single descriptor binding. It can be relative to one or more resources of
  // the same type.
  //
  struct Binding {
  ...
  DescriptorSetLayoutCreation& add_binding_at_index(const Binding& binding,
                                                    int index);
...
struct DescriptorSetCreation {
  ...
  DescriptorSetCreation& texture_sampler(
...
struct RenderPassOutput {
  ...
  RenderPassOutput& color(VkFormat format, VkImageLayout final_layout,
                          RenderPassOperation::Enum load_op);
  RenderPassOutput& depth_stencil(VkFormat format, VkImageLayout final_layout);
  RenderPassOutput& set_depth_stencil_operations(
...
struct RenderPassCreation {
  ...
  RenderPassCreation& set_depth_stencil_operations(
      RenderPassOperation::Enum depth, RenderPassOperation::Enum stencil);
...
struct ExecutionBarrier {
  ...
  ExecutionBarrier& set(PipelineStage::Enum source,
...
struct Pipeline {
  ...
  DescriptorSetLayoutHandle
      descriptor_set_layout_handles[k_max_descriptor_set_layouts];
  u32 num_active_layouts = 0;
...
static VkFormat to_vk_vertex_format(VertexComponentFormat::Enum value) {
  // Float, Float2, Float3, Float4, Mat4, Byte, Byte4N, UByte, UByte4N, Short2,
  // Short2N, Short4, Short4N, Uint, Uint2, Uint4, Count
...
VkPipelineStageFlags2KHR util_determine_pipeline_stage_flags2(
    VkAccessFlags2KHR access_flags, QueueType::Enum queue_type);

void util_add_image_barrier(
    GpuDevice* gpu, VkCommandBuffer command_buffer, Texture* texture,
    ResourceState new_state, u32 base_mip_level, u32 mip_count, bool is_depth,
...
void util_add_image_barrier(
    GpuDevice* gpu, VkCommandBuffer command_buffer, VkImage image,
    ResourceState old_state, ResourceState new_state, u32 base_mip_level,
    u32 mip_count, bool is_depth, u32 source_family = VK_QUEUE_FAMILY_IGNORED,
    u32 destination_family = VK_QUEUE_FAMILY_IGNORED,
    QueueType::Enum source_queue_type = QueueType::Graphics,
    QueueType::Enum destination_queue_type = QueueType::Graphics);
