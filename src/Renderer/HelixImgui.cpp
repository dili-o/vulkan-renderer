...
Helix::FlatHashMap<Helix::ResourceHandle, Helix::ResourceHandle>
    g_texture_to_descriptor_set;
...
static cstring g_vertex_shader_code = {
    ...
    "layout( std140, set = 1, binding = 0 ) uniform LocalConstants { mat4 "
    "ProjMtx; };\n"
    "void main()\n"
    "{\n"
    "    Frag_UV = UV;\n"
...
static cstring g_fragment_shader_code_bindless = {
    ...
    "void main()\n"
    "{\n"
    "    Out_Color = Frag_Color * texture(textures[nonuniformEXT(texture_id)], "
    "Frag_UV.st);\n"
    "}\n"};
...
void ImGuiService::init(void* configuration) {
  ImGuiServiceConfiguration* imgui_config =
      (ImGuiServiceConfiguration*)configuration;
  ...
  TextureCreation texture_creation;  // = { pixels, ( u16 )width, ( u16 )height,
                                     // 1, 1, 0, TextureFormat::R8G8B8A8_UNORM,
  ...
  if (gpu->gpu_device_features & GpuDeviceFeature_BINDLESS) {
    shader_creation.set_name("ImGui")
        .add_stage(g_vertex_shader_code_bindless,
                   (uint32_t)strlen(g_vertex_shader_code_bindless),
                   VK_SHADER_STAGE_VERTEX_BIT)
        .add_stage(g_fragment_shader_code_bindless,
                   (uint32_t)strlen(g_fragment_shader_code_bindless),
                   VK_SHADER_STAGE_FRAGMENT_BIT);
  } else {
    shader_creation.set_name("ImGui")
        .add_stage(g_vertex_shader_code, (uint32_t)strlen(g_vertex_shader_code),
                   VK_SHADER_STAGE_VERTEX_BIT)
        .add_stage(g_fragment_shader_code,
                   (uint32_t)strlen(g_fragment_shader_code),
                   VK_SHADER_STAGE_FRAGMENT_BIT);
  }
  ...
  pipeline_creation.blend_state_creation.add_blend_state().set_color(
  ...
  pipeline_creation.vertex_input_creation.add_vertex_stream(
      {0, 20, VertexInputRate::PerVertex});
  ...
  g_descriptor_set_layout =
      gpu->create_descriptor_set_layout(descriptor_set_layout_creation);
  ...
  cb_creation
      .set(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, ResourceUsageType::Dynamic, 64)
      .set_name("CB_ImGui");
  ...
  // Old Map
  g_texture_to_descriptor_set.init(&MemoryService::instance()->system_allocator,
                                   4);
  g_texture_to_descriptor_set.insert(g_font_texture.index,
                                     g_ui_descriptor_set.index);
  ...
  ib_creation
      .set(VK_BUFFER_USAGE_INDEX_BUFFER_BIT, ResourceUsageType::Dynamic,
           g_ib_size)
      .set_name("IB_ImGui");
...
void ImGuiService::render(Helix::CommandBuffer& commands, bool use_secondary) {
  ...
  // todo: key
  commands.bind_pass(gpu->get_swapchain_pass(), gpu->get_current_framebuffer(),
                     use_secondary);
  ...
  // todo:map
  DescriptorSetHandle last_descriptor_set = {
      g_texture_to_descriptor_set.get(last_texture.index)};
      ...
      if (pcmd->UserCallback) {
        ...
        if (clip_rect.x < fb_width && clip_rect.y < fb_height &&
          ...
          if (clip_origin_lower_left) {
            Rect2DInt scissor_rect = {(int16_t)clip_rect.x,
          ...
          if (!gpu->gpu_device_features & GpuDeviceFeature_BINDLESS) {
            if (new_texture.index != last_texture.index &&
              ...
              FlatHashMapIterator it =
                  g_texture_to_descriptor_set.find(last_texture.index);

              // TODO: invalidate handles and update descriptor set when needed
              // ? Found this problem when reusing the handle from a previous If
              // not present
              if (it.is_invalid()) {
                ...
                g_texture_to_descriptor_set.insert(new_texture.index,
                                                   last_descriptor_set.index);
              } else {
...
void ImGuiService::remove_cached_texture(Helix::TextureHandle& texture) {
  ...
  if (it.is_valid()) {
    ...
    Helix::DescriptorSetHandle descriptor_set{
        g_texture_to_descriptor_set.get(it)};
...
bool imgui_file_dialog_open( cstring button_name, cstring path, cstring
extension ) {
    ...
    if ( opened && ImGui::Begin( "helix_imgui_file_dialog", &opened,
ImGuiWindowFlags_AlwaysAutoResize ) ) {
        ...
        Helix::MemoryAllocator* allocator =
Helix::memory_get_system_allocator();
        ...
        if ( scan_folder ) {
            ...
            Helix::file_find_files_in_path( extension, directory.path, files,
directories );
        }
        ...
        for ( size_t d = 0; d < directories.get_string_count(); ++d ) {
            ...
            if ( ImGui::Selectable( directory_name, selected,
ImGuiSelectableFlags_AllowDoubleClick ) ) {
        ...
        for ( size_t f = 0; f < files.get_string_count(); ++f ) {
            ...
            if ( ImGui::Selectable( file_name, selected,
ImGuiSelectableFlags_AllowDoubleClick ) ) {
...
Helix::memory_get_system_allocator();
        ...
        if ( scan_folder ) {
            ...
            Helix::file_find_files_in_path( ".", directory.path, files,
directories );
        }
  ...
  void Draw(cstring title, bool* p_open = NULL) {
    ...
    if (Filter.IsActive()) {
      ...
      for (int line_no = 0; line_no < LineOffsets.Size; line_no++) {
        ...
        cstring line_end = (line_no + 1 < LineOffsets.Size)
...
static void imgui_print(cstring text) { s_imgui_log.AddLog("%s", text); }
...
class Sparkline {
 public:
  ...
  void draw(char const* const label = "", ImVec2 const size = ImVec2()) const {
    ...
    ImGui::PlotLines(label, getValue, const_cast<Sparkline*>(this), L, 0,
                     overlay, _min, _max, size);
  }
 ...
 protected:
  ...
  static void print(char* const buffer, size_t const bufferLen,
                    int const value) {
