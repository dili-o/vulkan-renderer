...
namespace Helix {
    ...
    void MeshLateCullingPass::render(CommandBuffer* gpu_commands, Scene* scene_) {
        ...
        //const Buffer* mesh_draw_count_buffer = renderer->gpu->access_buffer(scene->mesh_draw_count_buffers[buffer_frame_index]);
    ...
    void GBufferLatePass::prepare_draws(glTFScene& scene, FrameGraph* frame_graph, Allocator* resident_allocator) {
        renderer = scene.renderer;
    ...
    void DebugPass::prepare_draws(glTFScene& scene, FrameGraph* frame_graph, Allocator* resident_allocator) {
        if (renderer)
            return;
        renderer = scene.renderer;
    ...
    void glTFScene::load(cstring filename, cstring path, Allocator* resident_allocator, StackAllocator* temp_allocator, AsynchronousLoader* async_loader) {
        ...
        node_parents.init(temp_allocator, gltf_scene.nodes_count, gltf_scene.nodes_count);

        Array<glm::mat4> node_matrix;
        node_matrix.init(temp_allocator, gltf_scene.nodes_count, gltf_scene.nodes_count);
        ...
        node_handles.init(temp_allocator, gltf_scene.nodes_count, gltf_scene.nodes_count);
        ...
        while (node_stack.size) {
            ...
            for (u32 primitive_index = 0; primitive_index < gltf_mesh.primitives_count; ++primitive_index) {
                ...
                // Vertex positions
                glTF::Accessor& position_accessor = gltf_scene.accessors[position_accessor_index];
                glTF::BufferView& position_buffer_view = gltf_scene.buffer_views[position_accessor.buffer_view];
                ...
                //if(tangents)
                //    meshopt_optimizeVertexFetch(tangents, indices, indices_accessor.count, tangents, position_accessor.count, sizeof(glm::vec4));
                //if(tex_coords)
                //    meshopt_optimizeVertexFetch(tex_coords, indices, indices_accessor.count, tex_coords, position_accessor.count, sizeof(glm::vec2));
