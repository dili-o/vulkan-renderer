...
struct GPUTimestampManager {
  ...
  u32 resolve(u32 current_frame,
              GPUTimestamp* timestamps_to_fill);  // Returns the total queries
                                                  // for this frame.

  u32 push(u32 current_frame,
           cstring name);  // Returns the timestamp query index.
  ...
  bool current_frame_resolved =
      false;  // Used to query the GPU only once per frame if get_gpu_timestamps
...
struct DeviceCreation {
  ...
  void* window =
      nullptr;  // Pointer to API-specific window: SDL_Window, GLFWWindow
  u16 width = 1;
  u16 height = 1;
...
inline GpuDeviceFeature operator|(GpuDeviceFeature a, GpuDeviceFeature b) {
  return static_cast<GpuDeviceFeature>(static_cast<int>(a) |
                                       static_cast<int>(b));
}
inline GpuDeviceFeature operator|=(GpuDeviceFeature& a, GpuDeviceFeature b) {
  a = a | b;
...
struct GpuDevice : public Service {
  ...
  static void fill_write_descriptor_sets(
      GpuDevice& gpu, const DesciptorSetLayout* descriptor_set_layout,
      ...
      const ResourceHandle* resources, const SamplerHandle* samplers,
      const u16* bindings);
  ...
  DescriptorSetHandle create_descriptor_set(
  ...
  void query_pipeline(PipelineHandle handle,
                      PipelineDescription& out_description);
  void query_sampler(SamplerHandle handle, SamplerDescription& out_description);
  void query_descriptor_set_layout(
      DescriptorSetLayoutHandle handle,
      DescriptorSetLayoutDescription& out_description);
  void query_descriptor_set(DescriptorSetHandle handle,
                            DesciptorSetDescription& out_description);
  void query_shader_state(ShaderStateHandle handle,
                          ShaderStateDescription& out_description);

  const RenderPassOutput& get_render_pass_output(
      RenderPassHandle render_pass) const;

  // Update/Reload resources ///////////////////////////////////////////
  void resize_output_textures(FramebufferHandle framebuffer, u32 width,
                              u32 height);
  ...
  // Misc //////////////////////////////////////////////////////////////
  void link_texture_sampler(
      TextureHandle texture,
      SamplerHandle sampler);  // TODO: for now specify a sampler for a texture
  ...
  VkShaderModuleCreateInfo compile_shader(cstring code, u32 code_size,
  ...
  void queue_command_buffer(
      CommandBuffer* command_buffer);  // Queue command buffer that will not be
  ...
  void fill_barrier(RenderPassHandle render_pass,
                    ExecutionBarrier& out_barrier);

  BufferHandle get_fullscreen_vertex_buffer()
      const;  // Returns a vertex buffer usable for fullscreen shaders that uses
              // no vertices.
  RenderPassHandle get_swapchain_pass()
      const;  // Returns what is considered the final pass that writes to the
              // swapchain.
  ...
  const RenderPassOutput& get_swapchain_output() const {
    return swapchain_pass_output;
  }

  VkRenderPass get_vulkan_render_pass(const RenderPassOutput& output,
                                      cstring name);
  ...
  // Names and markers /////////////////////////////////////////////////
  void set_resource_name(VkObjectType object_type, uint64_t handle,
                         cstring name);
  ...
  RenderPassOutput
      swapchain_pass_output;  // TODO: Right now these are not used becuase
                              // RenderPassOutputs are generated.
  ...
  VkDescriptorSetLayout
      vulkan_bindless_descriptor_layout;  // Global bindless descriptor layout.
  VkDescriptorSet
      vulkan_bindless_descriptor_set;  // Global bindless descriptor set.
  ...
  DesciptorSetLayout* access_descriptor_set_layout(
      DescriptorSetLayoutHandle layout);
  const DesciptorSetLayout* access_descriptor_set_layout(
      DescriptorSetLayoutHandle layout) const;

  DescriptorSetLayoutHandle get_descriptor_set_layout(
      PipelineHandle pipeline_handle, int layout_index);
  DescriptorSetLayoutHandle get_descriptor_set_layout(
      PipelineHandle pipeline_handle, int layout_index) const;
