...
void CommandBuffer::reset() {
  ...
  for (u32 i = 0; i < resource_count; ++i) {
    DescriptorSet* v_descriptor_set =
        (DescriptorSet*)descriptor_sets.access_resource(i);

    if (v_descriptor_set) {
      // Contains the allocation for all the resources, binding and samplers
      // arrays.
...
void CommandBuffer::init(GpuDevice* gpu) {
  ...
  VkResult result = vkCreateDescriptorPool(device->vulkan_device, &pool_info,
                                           device->vulkan_allocation_callbacks,
                                           &vk_descriptor_pool);
  HASSERT(result == VK_SUCCESS);

  descriptor_sets.init(device->allocator, k_descriptor_sets_pool_size,
...
DescriptorSetHandle CommandBuffer::create_descriptor_set(
    const DescriptorSetCreation& creation) {
  ZoneScoped;
  ...
  DescriptorSet* descriptor_set =
      (DescriptorSet*)descriptor_sets.access_resource(handle.index);
  const DesciptorSetLayout* descriptor_set_layout =
      device->access_descriptor_set_layout(creation.layout);
  ...
  VkResult result = vkAllocateDescriptorSets(
      device->vulkan_device, &alloc_info, &descriptor_set->vk_descriptor_set);
  HASSERT(result == VK_SUCCESS);

  // Cache data
  u8* memory =
      hallocam((sizeof(ResourceHandle) + sizeof(SamplerHandle) + sizeof(u16)) *
                   creation.num_resources,
               device->allocator);
  descriptor_set->resources = (ResourceHandle*)memory;
  descriptor_set->samplers =
      (SamplerHandle*)(memory +
                       sizeof(ResourceHandle) * creation.num_resources);
  descriptor_set->bindings =
      (u16*)(memory + (sizeof(ResourceHandle) + sizeof(SamplerHandle)) *
                          creation.num_resources);
  ...
  GpuDevice::fill_write_descriptor_sets(
      *device, descriptor_set_layout, descriptor_set->vk_descriptor_set,
      descriptor_write, buffer_info, image_info, vk_default_sampler->vk_handle,
  ...
  vkUpdateDescriptorSets(device->vulkan_device, num_resources, descriptor_write,
                         0, nullptr);
    ...
    if (render_pass != current_render_pass) {
      if (device->gpu_device_features & GpuDeviceFeature_DYNAMIC_RENDERING) {
        VkRenderingAttachmentInfo
            color_attachments_info[8];  // max_number of attachments is 8
        memset(color_attachments_info, 0,
               sizeof(VkRenderingAttachmentInfo) *
                   framebuffer->num_color_attachments);

        for (u32 a = 0; a < framebuffer->num_color_attachments; ++a) {
          Texture* texture =
              device->access_texture(framebuffer->color_attachments[a]);
          ...
          color_attachment_info.imageLayout =
              device->gpu_device_features & GpuDeviceFeature_SYNCHRONIZATION2
          ...
          color_attachment_info.clearValue =
              render_pass->output.color_operations[a] ==
        ...
        bool has_depth_attachment =
            framebuffer->depth_stencil_attachment.index != k_invalid_index;

        if (has_depth_attachment) {
          Texture* texture =
              device->access_texture(framebuffer->depth_stencil_attachment);
          ...
          depth_attachment_info.clearValue =
              render_pass->output.depth_operation ==
                      RenderPassOperation::Enum::Clear
                  ? clears[k_depth_stencil_clear_index]
                  : VkClearValue{};
        }

        VkRenderingInfo rendering_info{VK_STRUCTURE_TYPE_RENDERING_INFO};
        rendering_info.flags =
            use_secondary ? VK_RENDERING_CONTENTS_SECONDARY_COMMAND_BUFFERS_BIT
                          : 0;
        rendering_info.renderArea = {0, 0, framebuffer->width,
                                     framebuffer->height};
        ...
        rendering_info.pColorAttachments =
            framebuffer->num_color_attachments > 0 ? color_attachments_info
                                                   : nullptr;
        rendering_info.pDepthAttachment =
            has_depth_attachment ? &depth_attachment_info : nullptr;
        ...
        render_pass_begin.renderArea.extent = {framebuffer->width,
                                               framebuffer->height};
        ...
        vkCmdBeginRenderPass(vk_handle, &render_pass_begin,
                             use_secondary
...
void CommandBuffer::bind_vertex_buffer(BufferHandle handle_, u32 binding,
                                       u32 offset) {
...
void CommandBuffer::bind_descriptor_set(DescriptorSetHandle* handles,
                                        u32 num_lists, u32* offsets,
                                        u32 num_offsets) {
  u32 offsets_cache[8];
  num_offsets = 0;
  ...
  vkCmdBindDescriptorSets(vk_handle, current_pipeline->vk_bind_point,
                          current_pipeline->vk_pipeline_layout, k_first_set,
                          num_lists, vk_descriptor_sets, num_offsets,
                          offsets_cache);

  if (device->gpu_device_features & GpuDeviceFeature_BINDLESS) {
    vkCmdBindDescriptorSets(vk_handle, current_pipeline->vk_bind_point,
                            current_pipeline->vk_pipeline_layout, 0, 1,
                            &device->vulkan_bindless_descriptor_set, 0,
                            nullptr);
  }
}

void CommandBuffer::bind_local_descriptor_set(DescriptorSetHandle* handles,
                                              u32 num_lists, u32* offsets,
                                              u32 num_offsets) {
  ...
  if (device->gpu_device_features & GpuDeviceFeature_BINDLESS) {
    vkCmdBindDescriptorSets(vk_handle, current_pipeline->vk_bind_point,
                            current_pipeline->vk_pipeline_layout, 1, 1,
                            &device->vulkan_bindless_descriptor_set, 0,
                            nullptr);
  }
}
...
void CommandBuffer::set_viewport(const Viewport* viewport) {
  ...
  if (viewport) {
    ...
    // Invert Y with negative height and proper offset - Vulkan has unique
    // Clipping Y.
...
void CommandBuffer::draw_indexed(u32 index_count, u32 instance_count,
...
void CommandBuffer::draw_indirect(BufferHandle buffer_handle, u32 offset,
                                  u32 stride) {
  Buffer* buffer = device->access_buffer(buffer_handle);
  ...
  vkCmdDrawIndirect(vk_handle, vk_buffer, vk_offset, 1,
                    sizeof(VkDrawIndirectCommand));
}

void CommandBuffer::draw_indexed_indirect(BufferHandle buffer_handle,
  ...
  vkCmdDrawIndexedIndirect(vk_handle, vk_buffer, vk_offset, 1,
                           sizeof(VkDrawIndirectCommand));
}
#if NVIDIA
...
void CommandBuffer::draw_mesh_task_indirect_count(BufferHandle command_buffer,
                                                  u32 command_offset,
  ...
  device->cmd_draw_mesh_tasks_indirect_count(
      vk_handle, command_buffer_->vk_handle, command_offset,
      count_buffer_->vk_handle, count_offset, max_draws, stride);
}
...
// DrawIndirect = 0, VertexInput = 1, VertexShader = 2, FragmentShader = 3,
// RenderTarget = 4, ComputeShader = 5, Transfer = 6
...
void CommandBuffer::fill_buffer(BufferHandle buffer, u32 offset, u32 size,
                                u32 data) {
  Buffer* vk_buffer = device->access_buffer(buffer);

  vkCmdFillBuffer(vk_handle, vk_buffer->vk_handle, VkDeviceSize(offset),
                  size ? VkDeviceSize(size) : VkDeviceSize(vk_buffer->size),
                  data);
}
...
void CommandBuffer::upload_texture_data(TextureHandle texture_handle,
                                        void* texture_data,
  ...
  util_add_image_barrier(device, vk_handle, texture, RESOURCE_STATE_COPY_DEST,
                         0, 1, false);
  // Copy from the staging buffer to the image
  vkCmdCopyBufferToImage(vk_handle, staging_buffer->vk_handle,
                         texture->vk_image,
                         VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &region);

  // Post copy memory barrier
  util_add_image_barrier(device, vk_handle, texture, RESOURCE_STATE_COPY_SOURCE,
                         0, 1, false, device->vulkan_transfer_queue_family,
                         device->vulkan_main_queue_family,
                         QueueType::CopyTransfer, QueueType::Graphics);
}

void CommandBuffer::upload_buffer_data(BufferHandle buffer_handle,
                                       void* buffer_data,
                                       BufferHandle staging_buffer_handle,
                                       sizet staging_buffer_offset) {
  Buffer* buffer = device->access_buffer(buffer_handle);
  ...
  vkCmdCopyBuffer(vk_handle, staging_buffer->vk_handle, buffer->vk_handle, 1,
                  &region);

  util_add_buffer_barrier_ext(
      device, vk_handle, buffer->vk_handle, RESOURCE_STATE_COPY_DEST,
...
void CommandBufferManager::init(GpuDevice* gpu_, u32 num_threads) {
  ...
  const u32 total_secondary_buffers =
      total_pools * k_secondary_command_buffers_count;
  ...
  for (u32 i = 0; i < total_buffers; i++) {
    ...
    const u32 frame_index =
        i / (num_command_buffers_per_thread * num_pools_per_frame);
    const u32 thread_index =
        (i / num_command_buffers_per_thread) % num_pools_per_frame;
    const u32 pool_index = pool_from_indices(frame_index, thread_index);
    // rprint( "Indices i:%u f:%u t:%u p:%u\n", i, frame_index, thread_index,
    // pool_index );
    ...
    vkAllocateCommandBuffers(gpu->vulkan_device, &cmd,
                             &current_command_buffer.vk_handle);
  ...
  for (u32 pool_index = 0; pool_index < total_pools; ++pool_index) {
    ...
    for (u32 scb_index = 0; scb_index < k_secondary_command_buffers_count;
         ++scb_index) {
      CommandBuffer cb{};
...
void CommandBufferManager::shutdown() {
  const u32 total_pools = num_pools_per_frame * k_max_frames;
  for (u32 i = 0; i < total_pools; i++) {
    vkDestroyCommandPool(gpu->vulkan_device, vulkan_command_pools[i],
                         gpu->vulkan_allocation_callbacks);
  }
...
CommandBuffer* CommandBufferManager::get_command_buffer(u32 frame,
                                                        u32 thread_index,
                                                        bool begin) {
  ...
  CommandBuffer* cb =
      &command_buffers[(pool_index * num_command_buffers_per_thread) +
                       current_used_buffer];
  if (begin) {
    cb->reset();
    cb->begin();
...
CommandBuffer* CommandBufferManager::get_secondary_command_buffer(
    u32 frame, u32 thread_index) {
  const u32 pool_index = pool_from_indices(frame, thread_index);
  ...
  CommandBuffer* cb =
      &secondary_command_buffers[(pool_index *
                                  k_secondary_command_buffers_count) +
                                 current_used_buffer];
  return cb;
}
