...
void GPUProfiler::init(Allocator* allocator_, u32 max_frames_) {
  ...
  timestamps =
      (GPUTimestamp*)halloca(sizeof(GPUTimestamp) * max_frames * 32, allocator);
...
void GPUProfiler::update(GpuDevice& gpu) {
  ...
  u32 active_timestamps =
      gpu.get_gpu_timestamps(&timestamps[32 * current_frame]);
...
void GPUProfiler::imgui_draw() {
...

  {
    ...
    draw_list->AddText({cursor_pos.x, cursor_pos.y + widget_height / 2.f},
                       0xff00ffff, buf);
    draw_list->AddLine(
        {cursor_pos.x + rect_width, cursor_pos.y + widget_height / 2.f},
    ...
    for (u32 i = 0; i < max_frames; ++i) {
      ...
      // drawList->AddRectFilled( { frame_x, cursor_pos.y + rect_height }, {
      // frame_x + rect_width, cursor_pos.y }, 0xffffffff );
      ...
      for (u32 j = 0; j < per_frame_active[frame_index]; ++j) {
        ...
        draw_list->AddRectFilled(
            {frame_x, cursor_pos.y + widget_height - rect_height},
      ...
      draw_list->AddLine({frame_x, cursor_pos.y + widget_height},
                         {frame_x, cursor_pos.y}, 0x0fffffff);

      // Debug only
      /*static char buf[ 32 ];
      sprintf( buf, "%u", frame_index );
      draw_list->AddText( { frame_x, cursor_pos.y + widget_height - 64 },
      0xffffffff, buf );

      sprintf( buf, "%u", frame_timestamps[0].frame_index );
      drawList->AddText( { frame_x, cursor_pos.y + widget_height - 32 },
      0xffffffff, buf );*/
    ...
    selected_frame = selected_frame == -1 ? (current_frame - 1) % max_frames
                                          : selected_frame;
    if (selected_frame >= 0) {
      ...
      for (u32 j = 0; j < per_frame_active[selected_frame]; ++j) {
        ...
        sprintf(buf, "(%2.4f)-%s %d", timestamp.elapsed_ms, timestamp.name,
                timestamp.depth);
  ...
  static cstring items[] = {"200ms", "100ms", "66ms", "33ms",
                            "16ms",  "8ms",   "4ms"};
  static const float max_durations[] = {200.f, 100.f, 66.f, 33.f,
                                        16.f,  8.f,   4.f};

  static int max_duration_index = 4;
  if (ImGui::Combo("Graph Max", &max_duration_index, items,
                   IM_ARRAYSIZE(items))) {
