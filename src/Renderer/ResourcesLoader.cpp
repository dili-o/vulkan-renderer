...
// Utility methods ////////////////////////////////////////////////////////
static void shader_concatenate(cstring filename,
                               Helix::StringBuffer& path_buffer,
                               Helix::StringBuffer& shader_buffer,
                               Helix::Allocator* temp_allocator);
static VkBlendFactor get_blend_factor(const std::string factor);
static VkBlendOp get_blend_op(const std::string op);
static void parse_gpu_pipeline(StringBuffer& resource_name_buffer,
                               nlohmann::json& pipeline, PipelineCreation& pc,
                               StringBuffer& path_buffer,
                               StringBuffer& shader_buffer,
                               Allocator* temp_allocator, Renderer* renderer,
                               FrameGraph* frame_graph,
                               StringBuffer& path_name_buffer);

// ResourcesLoader //////////////////////////////////////////////////
void ResourcesLoader::init(Helix::Renderer* renderer_,
                           Helix::StackAllocator* temp_allocator_,
                           Helix::FrameGraph* frame_graph_) {
  renderer = renderer_;
  temp_allocator = temp_allocator_;
  frame_graph = frame_graph_;
  resource_name_buffer.init(hkilo(64),
                            &MemoryService::instance()->system_allocator);
}

void ResourcesLoader::shutdown() { resource_name_buffer.shutdown(); }
...
void ResourcesLoader::load_program(cstring json_path) {
  ...
  program_creation.name = resource_name_buffer.append_use_f(
      "%s", name_string.c_str());  // name_string.c_str();
  ...
  if (pipelines.is_array()) {
    ...
    for (u32 i = 0; i < pipeline_size; i++) {
      ...
      pipeline_creation.name =
          resource_name_buffer.append_use_f("%s", pipeline_name.c_str());
      ...
      if (inherit_from.is_string()) {
        ...
        for (u32 ii = 0; ii < pipeline_size; ++ii) {
          ...
          if (name == inherited_name) {
            parse_gpu_pipeline(resource_name_buffer, pipeline_i,
                               pipeline_creation, path_buffer,
                               shader_code_buffer, temp_allocator, renderer,
                               frame_graph, pass_name_buffer);
            break;
          }
        }
      }
      // TODO: Properly get the name of each shader
      parse_gpu_pipeline(resource_name_buffer, pipeline, pipeline_creation,
                         path_buffer, shader_code_buffer, temp_allocator,
                         renderer, frame_graph, pass_name_buffer);
...
void parse_gpu_pipeline(StringBuffer& resource_name_buffer,
                        nlohmann::json& pipeline, PipelineCreation& pc,
                        StringBuffer& path_buffer,
                        StringBuffer& shader_code_buffer,
                        Allocator* temp_allocator, Renderer* renderer,
    ...
    for (sizet s = 0; s < shaders.size(); ++s) {
      ...
      if (includes.is_array()) {
        for (sizet in = 0; in < includes.size(); ++in) {
          ...
          shader_concatenate(name.c_str(), path_buffer, shader_code_buffer,
                             temp_allocator);
        }
      }
      ...
      if (name == "vertex")
        pc.shader_state_creation.add_stage(code, (u32)strlen(code),
    ...
    for (sizet v = 0; v < vertex_inputs.size(); ++v) {
      ...
      if (attribute_format.is_string()) {
        ...
        for (u32 e = 0; e < VertexComponentFormat::Count; ++e) {
          VertexComponentFormat::Enum enum_value =
              (VertexComponentFormat::Enum)e;
  ...
  if (!depth.is_null()) {
    ...
    if (comparison.is_string()) {
      ...
      if (name == "less_or_equal") {
        pc.depth_stencil_creation.depth_comparison =
  ...
  if (!blend_states.is_null()) {
    for (sizet b = 0; b < blend_states.size(); ++b) {
      ...
      blend_state.set_color(get_blend_factor(src_colour),
                            get_blend_factor(dst_colour),
    ...
    if (node) {
      ...
      if (name == "swapchain") {
        ...
        const RenderPass* render_pass =
            renderer->gpu->access_render_pass(node->render_pass);

        pc.render_pass = render_pass->output;
      }
    } else {
      HWARN("Cannot find render pass {}. Defaulting to swapchain",
            name.c_str());
      pc.render_pass = renderer->gpu->get_swapchain_output();
    }
  }
...
void shader_concatenate(cstring filename, StringBuffer& path_buffer,
                        StringBuffer& shader_buffer,
                        Allocator* temp_allocator) {
  // Read file and concatenate it
  path_buffer.clear();
  cstring shader_path =
      path_buffer.append_use_f("%s%s", HELIX_SHADER_FOLDER, filename);
  FileReadResult shader_read_result =
      file_read_text(shader_path, temp_allocator);
  if (shader_read_result.data) {
    // Append without null termination and add termination later.
    shader_buffer.append_m(shader_read_result.data,
                           strlen(shader_read_result.data));
  } else {
