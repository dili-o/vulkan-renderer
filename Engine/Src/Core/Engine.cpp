...
void Engine::init() {
  MemorySysConfig mem_config{hmega(1000), hmega(1000)};
  PlatformConfiguration platform_config{1280, 720, "Sandbox"};
  JobSysConfiguration job_config{};

  LogSys::init();
  FileSys::init();
  InputSys::init();
  MemorySys::init(mem_config);
  EventSys::init();
  Platform::init(platform_config);

  job_config.allocator = MemorySys::system_allocator();
